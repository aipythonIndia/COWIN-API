...
pincodes_served_till_now_45 = []
pincodes_served_till_now_18 = []
...
def send_msg_on_telegram(msg, today_date):
    print ("Sending Messege on Telegram ....")
...
def get_availability_for_45(age_limit, available_capacity):
    ...
    if (age_limit == 45) and (available_capacity > 0):
        ...
        if (pincode not in pincodes_served_till_now_45) or (str(today_date) not in sent_dates_record):
            pincodes_served_till_now_45.append(pincode)
            send_msg_on_telegram(msg, today_date)
            # print (msg) # For local testing
        else:
            print (f"Corresponding msg for Age 45+ for Pincode {pincode} has already been served on {today_date}")
    else:
        vaccine_found_status_45 = False
...
def get_availability_for_18(age_limit, available_capacity):
    ...
    if (age_limit == 18) and (available_capacity > 0):
        ...
        if (pincode not in pincodes_served_till_now_18) or (str(today_date) not in sent_dates_record):        
            pincodes_served_till_now_18.append(pincode)
            send_msg_on_telegram(msg,today_date)
            print (msg) # For local testing
        else:
            print (f"Corresponding msg for Age 18+ for Pincode {pincode} has already been served on {today_date}")
    else:
        vaccine_found_status_18 = False
